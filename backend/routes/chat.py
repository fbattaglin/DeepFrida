...
def build_history_payload(messages: list[dict[str, Any]], system_prompt: str) -> list[dict[str, str]]:
...
def build_request_payload(payload: ChatRequest, messages: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "model": payload.model,
        "messages": build_history_payload(messages, payload.system_prompt),
        "stream": True,
...
async def chat_route(payload: ChatRequest, request: Request) -> StreamingResponse:
    async def event_stream():
        ...
        log_inference_event(
            ...
            system_prompt_preview=prompt_preview(payload.system_prompt),
        )
        ...
        request_payload = build_request_payload(payload, history)
